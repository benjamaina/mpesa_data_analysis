...
Each chart fails gracefully — a broken chart returns a
placeholder PNG rather than crashing the endpoint.
"""
...
def _error_png(message: str) -> bytes:
    """Return a plain PNG with an error message instead of crashing."""
...
def chart_monthly_flow(monthly_summary: list[dict]) -> bytes:
    try:
        if not monthly_summary:
            return _error_png("No monthly data available")

        df     = pd.DataFrame(monthly_summary)
        months = df["month"].tolist()
        x, w   = range(len(months)), 0.35
        ...
        logger.error(f"chart_monthly_flow failed: {e}")
        return _error_png(str(e))
...
def chart_spending_category(spending_by_category: list[dict]) -> bytes:
    try:
        ...
        if df.empty:
            return _error_png("No spending data available")

        fig, ax = plt.subplots(figsize=(8, 8), facecolor=BG_COLOR)
        ax.set_facecolor(BG_COLOR)
        _, _, autotexts = ax.pie(
            df["total_spent"], labels=df["category"],
        ...
        logger.error(f"chart_spending_category failed: {e}")
        return _error_png(str(e))


def chart_balance_trend(balance_trend: list[dict]) -> bytes:
    try:
        if not balance_trend:
            return _error_png("No balance data available")
        ...
        logger.error(f"chart_balance_trend failed: {e}")
        return _error_png(str(e))
...
def chart_spending_day(spending_by_day: list[dict]) -> bytes:
    try:
        ...
        logger.error(f"chart_spending_day failed: {e}")
        return _error_png(str(e))
...
def render_chart(chart_name: str, analysis: dict) -> bytes:
    if chart_name not in CHART_REGISTRY:
        raise KeyError(f"Unknown chart '{chart_name}'. Available: {list(CHART_REGISTRY.keys())}")
    fn, data_key = CHART_REGISTRY[chart_name]
    return fn(analysis.get(data_key, []))